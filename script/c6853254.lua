--A Dragon Lord's Rising & Rebirth
local s,id=GetID()
...
function s.filter(c,e,tp)
	return c:IsSetCard(0x41a) and c:IsLevelBelow(7) and c:IsCanBeSpecialSummoned(e,0,tp,false,false)
end
function s.filter2(c,e,tp)
	return c:IsSetCard(0x41a) and c:IsLevelBelow(4) and c:IsCanBeSpecialSummoned(e,0,tp,false,false)
end
...
function s.negcon(e,tp,eg,ep,ev,re,r,rp)
	return aux.exccon(e) and Duel.IsExistingMatchingCard(aux.FaceupFilter(Card.c:IsSetCard(0x41a) and c:IsLevelBelow(7)),tp,LOCATION_ONFIELD,0,1,nil)
end
