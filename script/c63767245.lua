...
s.listed_series={0x95,0x48,0x406,0x41a}
...
function s.filter1(c,e,tp)
	...
	return (#pg<=0 or (#pg==1 and pg:IsContains(c))) and rk>0 and c:IsFaceup() and (c:IsSetCard(0x48) or c:IsSetCard(0x41a)) and (rk>0 or c:IsStatus(STATUS_NO_LEVEL))
...
function s.filter2(c,e,tp,mc,rk,pg)
	...
	return c:IsType(TYPE_XYZ) and mc:IsType(TYPE_XYZ,c,SUMMON_TYPE_XYZ,tp) and c:IsRank(rk) and (c:IsSetCard(0x48) or c:IsSetCard(0x41a)) and mc:IsCanBeXyzMaterial(c,tp)
