--Ranker's Comeback
local s,id=GetID()
function c29649321.initial_effect(c)
	--Activate
...
s.listed_series={0x42a}

function s.filter1(c,e,tp)
	local pg=aux.GetMustBeMaterialGroup(tp,Group.FromCards(c),tp,nil,nil,REASON_XYZ)
	return #pg<=1 and c:IsRankBelow(8) and (c:GetRank()>0 or c:IsStatus(STATUS_NO_LEVEL)) and c:IsCanBeSpecialSummoned(e,0,tp,false,false)
